...
if geteuid() == 0:
    print(
        "Must be run as user to receive notifications. Add user to group 'input', `sudo usermod -a -G input $USER`."
    )
    exit()
...
if eventX == None:
    print(
        'no device "HDA Intel PCH Headphone" found, checkout output of running `evtest`'
    )
    exit()
...
for line in p.stdout:
    if mask.search(line.decode("utf-8")) != None:
        ...
        # add some delay
        sleep(0.2)
        call(["pactl", "set-source-volume", "0", "400%"])
        # pactl set-source-volume 0 "400%"
        # call(['amixer', 'set', 'Internal Mic Boost', '90%'])
