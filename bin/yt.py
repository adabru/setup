...
if len(sys.argv) == 2:
  subprocess.run('youtube-dl -F {:}'.format(sys.argv[1]).split(), encoding='UTF-8')
elif len(sys.argv) == 4:
  subprocess.run('youtube-dl -f {:} -o _tmp_%(id)s.%(ext)s {:}'.format(sys.argv[2], sys.argv[1]).split(), encoding='UTF-8')
